...
public class AbstractGroupWithWhenValidatorTest {
    ...
    private static final int GROUP_WHEN_ENUM_INDEX = 2;
    private static final int GROUP_EXECUTE_WHEN_INDEX = 3;
    ...
    private ValidGroupDefinitionWithWhenAnnotation[] createGroups(Object[][] groupDataArray, RulesEngine rulesEngine)
        ...
        for (int i = 0; i < groups.length; i++) {
            groups[i] = new ValidGroupDefinitionWithWhenAnnotation(i, groupDataArray[i][GROUP_GROUP_KEY_INDEX].toString(),
    ...
    private void assertRulesAfterExecution(Object[][] ruleDataArray, ValidGroupRuleWithValidGroupKey[] rules) {
        ...
        for (int i = 0; i < rules.length; i++) {
            Assert.assertEquals(new StringBuffer().append("Rule: ").append(i).toString(),
                    ((Boolean) ruleDataArray[i][RULE_EXECUTE_WHEN_INDEX]).booleanValue(), rules[i].isExcuteWhen());
            Assert.assertEquals(new StringBuffer().append("Rule: ").append(i).toString(),
                    ((Boolean) ruleDataArray[i][RULE_EXECUTE_THEN_INDEX]).booleanValue(), !rules[i].getExecuteThens().isEmpty());
        }
    }
